...
void EpubReaderActivity::loop() {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
    ...
    startActivityForResult(std::make_unique<EpubReaderMenuActivity>(
                               renderer, mappedInput, epub->getTitle(), chSynth ? chCur : currentPage,
                               chSynth ? chTot : totalPages,
                               bookProgressPercent, stableCur, stableTot, SETTINGS.orientation,
...
void EpubReaderActivity::computeStablePageNumbers(int& stableCurrent, int& stableTotal, int* chapterCurrent,
  ...
  if (!chapterCurrent || !chapterTotal || stableCurrent < 1) {
    return;
  }
  ...
  const uint32_t span =
      (s + 1 < n) ? (sp[static_cast<size_t>(s + 1)] - g0) : (idx.totalPages - g0 + 1u);
  if (span == 0 || span > static_cast<uint32_t>(std::numeric_limits<int>::max())) {
    return;
  }
  *chapterTotal = static_cast<int>(span);
  *chapterCurrent = std::clamp(stableCurrent - static_cast<int>(g0) + 1, 1, *chapterTotal);
}
